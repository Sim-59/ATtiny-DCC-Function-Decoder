...
//  Funktion an PB3 entfällt dafür
...
#define CV_LF_MODE  49   // Schlusslicht vorwärts
#define CV_LR_MODE  50   // Schlusslicht rückwärts
...
uint8_t cv29_byte, cv49_lf_cfg, cv50_lr_cfg;
...
// initialize the digital pins as an outputs
  ...
  cv49_lf_cfg = Dcc.getCV(CV_LF_MODE);
  cv50_lr_cfg = Dcc.getCV(CV_LR_MODE);
...
void notifyDccFunc(uint16_t Addr, DCC_ADDR_TYPE AddrType, FN_GROUP FuncGrp, uint8_t FuncState) {
    switch (FuncGrp) {
      case FN_0_4:
        if ((cv49_lf_cfg & 0x80) == 0) {                                                                // CV49-Bit7 = 1 schaltet die Richtungserkennung aus
          if ((cv49_lf_cfg & 0x40) == 0) digitalWrite(LF_PIN, richtung && (FuncState & FN_BIT_00) >> 4);// PB0 bei F0 in Fahrrichtung vorwärts, wenn nicht deaktiviert in CV49
        } else {
          digitalWrite(LF_PIN, (FuncState & FN_BIT_00) >> 4);                                           // PB0 bei F0 in beiden Fahrtrichtungen
        }
        if ((cv50_lr_cfg & 0x80) == 0) {                                                                // CV50-Bit7 = 1 schaltet die Richtungserkennung aus
          if ((cv50_lr_cfg & 0x20) == 0) {
            if (cv34_f0r_map & 0x02) digitalWrite(LR_PIN, !richtung && (FuncState & FN_BIT_00) >> 4);   // PB4 bei F0 in Fahrrichtung rückwärts, wenn nicht deaktiviert in CV50
          ...
          if (cv38_f4map & 0x80) digitalWrite(LR_PIN, (FuncState & FN_BIT_04) >> 3);                    // PB4 bei F4 in beiden Fahrtrichtungen
        }
